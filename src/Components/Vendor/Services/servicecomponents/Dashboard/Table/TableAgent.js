...
const columns = [
  ...
  { id: 'labour', label: 'Labour',align: 'center', minWidth: 60 },
  { id: 'material', label: 'Material',align: 'center', minWidth: 120 },
...
function createData(service ,labour, material,gender,added,pending,action) {
  added=added.slice(0,10)
  return { service ,labour, material,gender, added,pending,action };
}
...
//   console.log(servicesInfo , 'Services')
  ...
  for(var i=0; i<servicesInfo?.length; i++) 
  {
    ...
    if(datapoint?.type === "Agents")
    {
    var temp = datapoint?.info
    
    for(var j = 0 ; j < temp?.length ; j++)
    {      
      rows.push(createData(datapoint?.type ,temp[j]?.labour ? "Required" : "Not required" ,temp[j]?.material ? temp[j]?.material : "__" ,temp[j]?.gender ? temp[j]?.gender : "__" , datapoint.created_at, map_of_["Agents"] ? map_of_["Agents"] + " Pending" : "0 Pending","action"))
    }
    }
