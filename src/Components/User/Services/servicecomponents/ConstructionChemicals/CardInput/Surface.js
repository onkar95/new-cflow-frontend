...
function Surface({setNewRequest,setOpenSaved,quantity,setQuantity,newRequest,setModalOpen,setData}) {
 
    const [userId,setUserId] = useState(JSON.parse(localStorage.getItem('profile'))?.data?.id)
    const history = useHistory()


  const types = ["Curing compounds","Deshuttering agents","Water repellent coating","Tile cleaner","Demoulding agents","Surface retarders","Rust removers","Concrete cleaners"]
  
  const brands =["Dimetrics MOD SUP","KCH","MXBON","ALSTA","CETEX","Asmi Chem","CORONASH","KK India","Star pass 1","Addage","Omet"]
