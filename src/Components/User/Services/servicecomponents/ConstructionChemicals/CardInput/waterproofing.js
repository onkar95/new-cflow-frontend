...
import "./Styles.css"
function Waterproofing({setNewRequest,setOpenSaved,quantity,setQuantity,newRequest,setModalOpen,setData}) {
 
    const [userId,setUserId] = useState(JSON.parse(localStorage.getItem('profile'))?.data?.id)
    const history = useHistory()
  const types = ["Integral waterproofing" , "Crystalline waterproofing" , "Coatings","Hydrophilic waterstop" , "Membranes" , "Cementitious waterproofing" , "Liquid waterproofing  membrane" , "Bituminous waterproofing" , "Polyeurethane liquid membrane waterproofing" , "Silicone waterepallant"]
  
  const brands = ["Master Seal","Dr.Fixit","Krytonite","Newcoat and Newcoat cool" , "Zentrifix - Elastic","KEM Proof 87" , "Peterseal" , "Proofex" , "Polyureasystem"]
